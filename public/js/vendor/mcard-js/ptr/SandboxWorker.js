...
async function executeCode(code, input, context) {
  // Wrap context with Python-style .get(key, default) method
  const ctx = context || {};
  const wrappedContext = {
    ...ctx,
    get: function(key, defaultValue = undefined) {
      return (key in ctx) ? ctx[key] : defaultValue;
    }
  };
  // Also add .get() to input for convenience
  const wrappedInput = {
    ...input,
    get: function(key, defaultValue = undefined) {
      return (key in input) ? input[key] : defaultValue;
    }
  };
  // Create a sandboxed function with both input and context having .get()
  const fn = new Function('input', 'context', code);
  return fn(wrappedInput, wrappedContext);
}
`;
/**
